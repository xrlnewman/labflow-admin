...
test('LabFlow has appointment, schedule and followup data', async()=>{const source=await readFile(new URL('../src/main.js',import.meta.url),'utf8'); assert.match(source,/今日样本批次队列/); assert.match(source,/实验员排班/); assert.match(source,/质控任务/); assert.match(source,/AP-0716-082/)})
