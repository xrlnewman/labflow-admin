...
const demoAppointments = [
  { id: 'AP-0716-082', patient: '林晓雨', department: '全科门诊', doctor: '林实验员', scheduledAt: '2026-07-16T09:30:00+08:00', status: '候诊中' },
  { id: 'AP-0716-081', patient: '沈明远', department: '皮肤科', doctor: '沈实验员', scheduledAt: '2026-07-16T09:45:00+08:00', status: '已签到' },
  { id: 'AP-0716-080', patient: '赵思涵', department: '康复理疗', doctor: '赵实验员', scheduledAt: '2026-07-16T10:00:00+08:00', status: '已完成' },
  { id: 'AP-0716-079', patient: '周子昂', department: '全科门诊', doctor: '林实验员', scheduledAt: '2026-07-16T10:15:00+08:00', status: '待签到' },
  { id: 'AP-0716-078', patient: '许安然', department: '营养咨询', doctor: '周实验员', scheduledAt: '2026-07-16T10:30:00+08:00', status: '待签到' },
]
...
const statusColors = { 待签到: 'coral', 已签到: 'indigo', 候诊中: 'amber', 检测中: 'green', 已完成: 'green', 已取消: 'gray' }
const nav = [
  ['overview', '运营总览', '⌂'],
  ['queue', '样本批次队列', '▤'],
  ['doctors', '实验员排班', '◉'],
...
function appointmentAction(appointment) {
  ...
  if (appointment.status === '候诊中') return `<button class="text-action" data-action="status" data-next-status="检测中" data-appointment-id="${appointment.id}">开始检测</button>`
...
async function refreshFromApi({ quiet = false } = {}) {
  ...
  try {
    ...
    dataSource = 'API 数据'
    if (!quiet) toast = '已从 LabFlow API 刷新数据'
  } catch (error) {
    dataSource = '演示数据'
...
async function createAppointment() {
  try {
    const created = await api.createAppointment({ patient: '移动端演示样本', patientId: 'PT-MOBILE-DEMO', department: '全科门诊', doctor: '林实验员', scheduledAt: new Date().toISOString() })
    appointments = [normalizeAppointment(created), ...appointments]
    dataSource = 'API 数据'
    showToast('样本批次已创建，可继续在移动端完成签到')
  } catch (error) {
    dataSource = '演示数据'
